...
Under the hood it implements 7 learning mechanisms: self-reflection, spike detection for anomalies, quality gates, IF/THEN rule extraction, cross-evaluation, contradiction resolution, and selective forgetting. The system even learns to learn better -- it tracks which reflections produce impactful rules and adjusts its own reflection strategy over time. Agent types (like `sales-sdr`, `support-agent`, `content-writer`) come pre-seeded with rules from the domain so your agent doesn't start from zero. It's framework-agnostic -- works with LangChain, CrewAI, raw API calls, whatever. The mechanisms are inspired by Reflexion (Shinn et al.), cognitive memory architectures, and a lot of trial and error running this in production.
